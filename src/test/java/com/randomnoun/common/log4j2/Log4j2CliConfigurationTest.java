...
/**
 * Unit test for Log4j2CliConfigurationTest
 *
 * @blog http://www.randomnoun.com/wp/2013/01/13/logging/
 * 
 * @author knoxg
 */
public class Log4j2CliConfigurationTest
	extends TestCase
{
	...
	public void testWithPrefix() {
		...
		Log4j2CliConfiguration lcc = new Log4j2CliConfiguration();
		lcc.init("[Log4j2CliConfigurationTest]", null);
		Logger logger = Logger.getLogger("testLogger");
		
		logger.info("info message");
		regex = "^\\[Log4j2CliConfigurationTest\\] [0-9]{2}:[0-9]{2}:[0-9]{2},[0-9]{3} INFO  testLogger - info message$";
		line = baos.toString().split(lineSeparator)[0];
		// should look something like "06:52:08,156 INFO  testLogger - info message"
		assertTrue("String '" + line + "' does not match regex '" + regex + "'", line.matches(regex));
		
		logger.debug("debug message");
		regex = "^\\[Log4j2CliConfigurationTest\\] [0-9]{2}:[0-9]{2}:[0-9]{2},[0-9]{3} DEBUG testLogger - debug message$";
		line = baos.toString().split(lineSeparator)[1];
		assertTrue("String '" + line + "' does not match regex '" + regex + "'", line.matches(regex));
	}
	...
	public void testPropertiesOverride() {
		...
		lcc.init("[Log4j2CliConfigurationTest]", props);
		Logger logger = Logger.getLogger("testLogger");
